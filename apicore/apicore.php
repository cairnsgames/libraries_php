...
{
	global $defaultwhere, $defaultsss, $defaultparams, $defaultorder, $appid;
	...
	if (array_key_exists("subkey", $info) && $info["subkey"] && $info["subkey"] != "count") {
		...
		if (!array_key_exists("subkeys", $config[$table])) {
			header($_SERVER['SERVER_PROTOCOL'] . UNPROCESSABLEENTITY, true, 422);
			die("Action not allowed, subkeys not available");
		}
		$subkey = $info["subkey"];

		if (!array_key_exists($subkey, $config[$table]["subkeys"])) {
			header($_SERVER['SERVER_PROTOCOL'] . UNPROCESSABLEENTITY, true, 422);
			die("Action not allowed, sub table ($subkey) not available");
		}
		...
		$keyfield = $config[$table]["subkeys"][$subkey]["key"];
		if (isset($tconfig["order"])) {
			$defaultorder = $tconfig['order'];
		}
		if (isset($tconfig["limit"])) {
	...
	if (is_array($tconfig["select"])) {
		...
		if (strpos($fieldlist, "{" . $keyfield . "}") != false) {
			$num = substr_count($fieldlist, "{" . $keyfield . "}");
			$sql = str_replace("{" . $keyfield . "}", "?", $fieldlist);
			$sss = "s";
			array_push($param, $info["key"]);
			while (strlen($sss) < $num) {
				$sss .= "s";
				array_push($param, $info["key"]);
			}
		} else {
			$sql = $config[$table]["select"];
		}
